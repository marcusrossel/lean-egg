...
-- TODO: Remove MetaM from the first two functions.

private partial def subexprsWithoutMVars (expr : Expr) (amb : MVars.Ambient) : MetaM ExprSet := do
  ...
  expr.foldlM (init := ∅) fun acc child => return acc.union (← subexprsWithoutMVars child amb)

private def deriveGuides (rw : Rewrite) (amb : MVars.Ambient) : MetaM ExprSet := do
  let mut result := ∅
...
def genDerivedGuides (rws : Rewrites) (amb : MVars.Ambient) : MetaM Guides := do
  let guides : ExprSet ← rws.foldlM (init := ∅) (return ·.union <| ← deriveGuides · amb)
  let mut result := #[]
