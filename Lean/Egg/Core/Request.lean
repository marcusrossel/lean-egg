...
def encoding (goal : Congr) (rws : Rewrites) (cfg : Config) (explode : MVars) : MetaM Request := do
  let mut rws ← rws.encode cfg.toEncoding explode
  if !explode.isEmpty then rws := rws ++ (← explosionExprSubsts)
  return {
...
def insert (req : Request) (rws : Rewrites.Encoded) : Request :=
  { req with rws := req.rws ++ rws }
