...
macro "char_two_ring" basket?:(ident)? mod:egg_cfg_mod guides:(egg_guides)? : tactic => `(tactic|
  egg $[$basket?]? $mod [comm_add, comm_mul, add_assoc, mul_assoc, sub_canon, neg_add, div_canon,
  zero_add, zero_mul, one_mul, distrib, pow_zero, pow_one, pow_two, pow_three, char_two] $[$guides]?
)
