...
attribute [egg rise] Function.comp Nat.add_zero Nat.add_succ Nat.mul_zero Nat.mul_succ
...
-- Note: We don't tag this with `@[egg rise]` as it is not a theorem which is suitable for pattern matching.
...
theorem id_to_transpose (as : Vec (Vec α m) n) : transpose (transpose as) = as := by
  ...
  all_goals egg +rise [*, fill_nil (_ ::ᵥ _)]
