...
def Lean.Meta.mkHCongrWithArity' (f : Expr) (numArgs : Nat) : MetaM CongrTheorem := do
...
#eval show TermElabM Unit from do
  ...
  let lamAB h := Expr.lam `a (.const `Nat []) (.lam `b (.const `Nat []) h .default) .default
  let proof ← mkFreshExprMVar none
  withLocalDecls #[(`a, .default, fun _ => return .const `Nat []), (`b, .default, fun _ => return .const `Nat [])] fun fvars => do
    let lhs := lamAB (← mkCHole true  (.app (.app (.const `Nat.add []) fvars[0]!) fvars[1]!) proof)
    let rhs := lamAB (← mkCHole false (.app (.app (.const `Nat.add []) fvars[1]!) fvars[0]!) proof)
...
#eval show TermElabM Unit from do
  let m₁ ← mkFreshExprMVar none .syntheticOpaque
  let m₂ ← mkFreshExprMVar none .syntheticOpaque
  let lhs := Expr.lam `a (.const `Nat []) (.app (.app (.const `Nat.add []) m₁) (mkNatLit 0)) .default
  let rhs := Expr.lam `a (.const `Nat []) (.app (.app (.const `Nat.add []) m₂) (mkNatLit 0)) .default
  let eq ← mkFreshExprMVar (← mkEq lhs rhs)
  let rws ← eq.mvarId!.congrN! (depth? := some 1)
  IO.println <| ← rws.mapM (fun rw => do ppExpr (← rw.getType))
  -- let [rw] ← eq.mvarId!.congrN | throwError "expected single goal"
  -- return rw.getType
