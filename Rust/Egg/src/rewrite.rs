...
fn eval_eq_condition(cond: &Pattern<LeanExpr>, graph: &mut LeanEGraph, subst: &Subst) -> bool {
    ...
    let ENodeOrVar::ENode(LeanExpr::App([i2, i9])) = &ast.index(i1) else { return false };
    let ENodeOrVar::ENode(LeanExpr::App([i3, i8_])) = &ast.index(*i2) else { return false };
    let ENodeOrVar::ENode(LeanExpr::App([i4, _i7])) = &ast.index(*i3) else { return false };
    let ENodeOrVar::ENode(LeanExpr::Const(b)) = &ast.index(*i4) else { return false };
    let [i5, ..] =  &**b else { return false };
    let ENodeOrVar::ENode(LeanExpr::Str(string)) = &ast.index(*i5) else { return false };
