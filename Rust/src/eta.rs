...
impl Applier<LeanExpr, LeanAnalysis> for Eta {
    ...
    fn apply_one(&self, egraph: &mut LeanEGraph, eta_class: Id, subst: &Subst, _: Option<&PatternAst<LeanExpr>>, rule: Symbol) -> Vec<Id> {
        ...
        let mut shifted_graph: LeanEGraph = Default::default();
        match eta_shift(0, fun_class, egraph, &mut shifted_graph, &mut HashMap::new(), rule) {
            ClassState::New(shifted_fun_class) => {
                egraph.egraph_union(&shifted_graph);
                // TODO: Is `shifted_fun_class` still valid in `egraph` because we unioned `shifted_graph` into it?
...
fn eta_shift(threshold: u64, target_class: Id, src_graph: &LeanEGraph, dst_graph: &mut LeanEGraph, state: &mut HashMap<Id, ClassState>, rule: Symbol) -> ClassState {
    // Optimization: If all of the target's e-nodes contain the bvar value that leads to removal, 
    // the whole class will be removed.
    if src_graph[target_class].data.bvars.contains(&threshold) {
    ...
    let mut nodes = src_graph[target_class].nodes.clone();
    ...
    'queue_loop: while let Some(node) = queue.pop_front() {
        match node {
            LeanExpr::BVar(e) => {
                ...
                let idx = src_graph[e].data.nat_val.unwrap();
                match idx.cmp(&threshold) {
                    Ordering::Less => {
                        let new_node = LeanExpr::Nat(idx); 
                        register_node(&mut new_class, new_node, dst_graph, state, target_class, rule)
                    }
                    Ordering::Greater => {
            ...
            LeanExpr::Lam([ty, body]) | LeanExpr::Forall([ty, body]) => {
                ...
                let s1 = eta_shift(threshold, ty, src_graph, dst_graph, state, rule);
                let s2 = eta_shift(threshold + 1, body, src_graph, dst_graph, state, rule);
                match (s1, s2) {
            ...
            LeanExpr::Const(es) => {
                ...
                for &e in es.iter() {
                    match eta_shift(threshold, e, src_graph, dst_graph, state, rule) {
            ...
            LeanExpr::App([e1, e2]) | LeanExpr::Max([e1, e2]) | LeanExpr::IMax([e1, e2]) => {
                // TODO: Is it a problem that we're exploring both paths before checking the result of
                //       the first (aside from being less efficient)?
                let s1 = eta_shift(threshold, e1, src_graph, dst_graph, state, rule);
                let s2 = eta_shift(threshold, e2, src_graph, dst_graph, state, rule);
                match (s1, s2) {
                    (ClassState::New(child1_class), ClassState::New(child2_class)) => {
                        let new_node = swap_children(&node, [child1_class, child2_class]);
                        register_node(&mut new_class, new_node, dst_graph, state, target_class, rule);
            ...
            LeanExpr::Nat(_) | LeanExpr::Str(_) | LeanExpr::Erased => 
                register_node(&mut new_class, node, dst_graph, state, target_class, rule)
        }
    }
...
fn register_node(new_class: &mut Option<Id>, new_node: LeanExpr, dst_graph: &mut LeanEGraph, state: &mut HashMap<Id, ClassState>, target_class: Id, rule: Symbol) {
    let node_class = dst_graph.add(new_node);
    match new_class {
        Some(id) => _ = dst_graph.union_trusted(*id, node_class, rule),
        None => {
