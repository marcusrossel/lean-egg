...
pub fn match_is_valid(
    subst: &Subst,
    pat: &PatternAst<LeanExpr>,
    graph: &LeanEGraph,
) -> MatchValidity {
    let mut ctx = Context {
        pat,
        subst,
        graph,
        parent_binders: HashMap::new(),
        binder_depths: HashMap::new(),
    };
    ...
    match &ctx.pat.as_ref()[idx] {
        ENodeOrVar::Var(var) => {
            ...
            if loose_bvars.is_empty() {
                ...
                if loose_bvars.iter().any(|b| *b < loc.binder_depth) {
                    return false;
                }
        ...
        ENodeOrVar::ENode(e) => {
            for (i, child) in e.children().iter().enumerate() {
                ...
                if !match_is_valid_core(child_idx, child_loc, ctx) {
                    return false;
                }
            }
            true
